"""Normalize judged datasets into release-style artifacts."""
...
def normalize_row(row: Dict[str, Any], judge_model: str) -> Dict[str, Any]:
...
def normalize_dataset(input_path: Path, output_path: Path, judge_model: str) -> Dict[str, Any]:
    source_rows = load_jsonl(input_path)
    normalized_rows = [normalize_row(row, judge_model) for row in source_rows]
    ...
    return {
        ...
        "field_presence": {
            ...
            "has_judge_model": all(bool(str(row.get("judge_model", "")).strip()) for row in normalized_rows),
        },
